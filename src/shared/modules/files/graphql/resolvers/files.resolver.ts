...
export class FilesResolver extends BaseResolver {
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['FILES'], action: ACTION_LIST.DELETE })
  @Mutation(() => GraphQLVoid, { nullable: true })
  async deleteFiles(
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['FILES'], action: ACTION_LIST.GET_ALL_OWN })
  @Query(() => [FilesResponse])
  async getAllFilesOwn(
    @Args('input', { nullable: true }) input: GetAllFilesInput,
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['FILES'], action: ACTION_LIST.GET_PAGINATED_OWN })
  @Query(() => PaginatedFilesResponse, { nullable: true })
  async getPaginatedFilesOwn(
    @Args('input', { nullable: true }) input: GetPaginatedFilesInput,
    @CurrentLanguage() lang?: string,
    @CurrentUser() user?: AuthUser,
  ): Promise<PaginatedFilesResponse> {
    return this.getPaginatedFiles({
      ...input,
      where: { ...input.where, createdBy: { eq: user.userId } },
    });
  }
  ...
  async createdBy(@Parent() parent?: FilesResponse): Promise<SolvedEntityResponse> {
    if (parent?.createdBy) {
  ...
  @ResolveField(() => [SolvedEntityResponse], { nullable: true })
  async updatedBy(@Parent() parent?: FilesResponse): Promise<SolvedEntityResponse> {
    if (parent?.updatedBy) {
      const userOrErr = await this._cqrsBus.execQuery<Result<UserEntity>>(new GetOneUserQuery({
        where: {
