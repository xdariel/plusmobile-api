...
export class RoleMapper implements BaseMapper<RoleEntity> {
  ...
  // @ts-ignore
  dtoInput2Persistent<DTO = CreateRoleInput>({ name, description, permits, onlyAdmin }: CreateRoleInput): RoleEntity {
    return {
  ...
  persistent2Dto(entity: RoleEntity): RoleResponse {
    return {
      ...entity,
      createdBy: entity?.createdBy ? { id: entity.createdBy } : undefined,
      updatedBy: entity?.updatedBy ? { id: entity.updatedBy } : undefined,
    };
  }
