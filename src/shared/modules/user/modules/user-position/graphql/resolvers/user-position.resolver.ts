...
export class UserPositionResolver extends BaseResolver {
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['USER_POSITION'], action: ACTION_LIST.DELETE })
  @Mutation(() => GraphQLVoid, { nullable: true })
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['USER_POSITION'], action: ACTION_LIST.GET_ALL_OWN })
  @Query(() => [UserPositionResponse])
  async getAllUserPositionOwn(
    @Args('input', { nullable: true }) input: GetAllUserPositionInput,
    @CurrentUser() user?: AuthUser,
    @CurrentLanguage() lang?: string,
  ): Promise<Array<UserPositionResponse>> {
    return this.getAllUserPosition({
      ...input,
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES['USER_POSITION'], action: ACTION_LIST.GET_PAGINATED_OWN })
  @Query(() => PaginatedUserPositionResponse, { nullable: true })
  async getPaginatedUserPositionOwn(
    @Args('input', { nullable: true }) input: GetPaginatedUserPositionInput,
    @CurrentLanguage() lang?: string,
    @CurrentUser() user?: AuthUser,
  ): Promise<PaginatedUserPositionResponse> {
    return this.getPaginatedUserPosition({
      ...input,
      where: { ...input.where, createdBy: { eq: user.userId } },
    });
  }
  ...
  async createdBy(@Parent() parent?: UserPositionResponse): Promise<SolvedEntityResponse> {
    if (parent?.createdBy) {
  ...
  @ResolveField(() => [SolvedEntityResponse], { nullable: true })
  async updatedBy(@Parent() parent?: UserPositionResponse): Promise<SolvedEntityResponse> {
    if (parent?.updatedBy) {
      const userOrErr = await this._cqrsBus.execQuery<Result<UserEntity>>(new GetOneUserQuery({
        where: {
