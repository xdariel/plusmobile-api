...
export class UserService extends BaseEntityService<UserEntity> implements IUserService {
  constructor(private readonly _userRepo: UserRepository,
              ...
              @Inject(DB_USER) dbUser: DBUser,
  ) {
    super(_userRepo, UserEntity.name, dbUser);
  }
  ...
  async updateById(id: Identifier, update: UserUpdateType): Promise<Result<void>> {
    ...
    if (update?.multiLevelInfo) {
      update.multiLevelInfo = merge(user.multiLevelInfo, update.multiLevelInfo);
      if (user.multiLevelInfo && !user.multiLevelInfo?.refCode) {
        update.multiLevelInfo.refCode = generate({ lowercase: false, uppercase: true, symbols: false, length: 7 });
      }
    }
