...
export class UserResolver extends BaseResolver {
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES.USERS, action: ACTION_LIST.DELETE })
  @Mutation(() => GraphQLVoid, { nullable: true })
  async deleteUser(
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES.USERS, action: ACTION_LIST.GET_ALL_OWN })
  @Query(() => [UserResponse])
  async getAllUserOwn(
    @Args('input', { nullable: true }) input: GetAllUsersInput,
    @CurrentUser() user?: AuthUser,
  ...
  @UseGuards(GqlAuthGuard, PermitsGuard)
  @Permit({ module: APP_MODULES.USERS, action: ACTION_LIST.GET_PAGINATED_OWN })
  @Query(() => PaginatedUsers, { nullable: true })
  async getPaginatedUserOwn(
    @Args('input', { nullable: true }) input: GetPaginatedUsersInput,
    @CurrentLanguage() lang?: string,
    @CurrentUser() user?: AuthUser,
  ): Promise<PaginatedUsers> {
    return this.getPaginatedUser({
      ...input,
      where: { ...input.where, createdBy: { eq: user.userId } },
    });
  }
  ...
  async createdBy(@Parent() parent?: UserResponse): Promise<SolvedEntityResponse> {
    if (parent?.createdBy) {
  ...
  @ResolveField(() => [SolvedEntityResponse], { nullable: true })
  async updatedBy(@Parent() parent?: UserResponse): Promise<SolvedEntityResponse> {
    if (parent?.updatedBy) {
      const userOrErr = await this._cqrsBus.execQuery<Result<UserEntity>>(new GetOneUserQuery({
        where: {
