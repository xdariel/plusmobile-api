...
////Roles////
  ...
  async area(@Parent() parent?: ProfileResponse): Promise<SolvedEntityResponse> {
    if (parent?.area) {
      const areaOrErr = await this._cqrsBus.execQuery<Result<UserAreaEntity>>(new GetOneUserAreaQuery({
        where: {
  ...
  async position(@Parent() parent?: ProfileResponse): Promise<SolvedEntityResponse> {
    if (parent?.position) {
      const positionOrErr = await this._cqrsBus.execQuery<Result<UserPositionEntity>>(new GetOneUserPositionQuery({
        where: {
